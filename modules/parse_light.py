...
def get_arg():
    ...
    parser.add_argument("-l","--log",action="store_true", default=False,help="This option will log all output and error messages to 'error.log'. Turned on by default.")
    ...
    #print(a)
    if a.log:
        print('All stdout and stderr messages will be redirected to error.log. Check it after program execution.')
...
#yn: [('label',labeln) ,('values',y_list))]
    ...
    printw('Results file is saved to '+plot_title+'.html',2)
...
if arg.storage=='3par':
    ...
    fh=open(fn)
    if 'statcpu' in fn:
        printw('This is 3par CPU stats file.',delay)
        printw('Processing...',delay)
        cpu_3par(fh)
    elif 'statvv' in fn:
        printw('This is 3par LUN stats file.',delay)
        printw('Processing...',delay)
        vv_3par(fh)
    else:
        print('I can not recognize it unfortunately. Please check the content of this file and its name.')
...
else: print('It took only ',run_time,' seconds to finish processing. Give me more data!')
