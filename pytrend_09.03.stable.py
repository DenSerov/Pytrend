...
if arg.storage=='3par':
    ...
    avg_latency=data['many'][2]
    lat_iops={}
    for i in range(len(iops)):
        lat_iops[iops[i]]=avg_latency['values'][i]

    lat_iops=collections.OrderedDict(sorted(lat_iops.items()))
    iops=list()
    for iop in lat_iops.keys():
        iops.append(iop)
    lat=list()
    for la in lat_iops.values():
        lat.append(la)
    avg_latency=dict([('label','Average Latency'),('values',lat)])

    plotmany(iops,[avg_latency],'Latency on IOPS')
...
if run_time>30: print('It took',run_time,'seconds to finish processing.')
